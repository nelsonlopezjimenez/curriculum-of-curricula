    ...
    if (Test-Path $appDataPath) {
        ...
        if (-not $isJunction) {
            try {
                ...
                cmd /c "mklink /J `"$appDataPath`" `"$AppDataTarget`"" | Out-Null
                Write-Log "AppData junction created → $AppDataTarget"
            } catch {
...
if (-not $isOsn) {
    ...
    if (-not (Test-Path $vscodePath)) {
        try {
            cmd /c "mklink /J `"$vscodePath`" `"$VscodeTarget`"" | Out-Null
            Write-Log ".vscode junction created → $VscodeTarget"
        } catch {
...
if (-not $isOsn) {
    ...
    foreach ($dot in $dotfiles) {
        ...
        try {
            cmd /c "mklink /J `"$userPath`" `"$sourcePath`"" | Out-Null
            Write-Log "$dot junction created → $sourcePath"
        } catch {
...
if (-not $isOsn) {
    ...
    foreach ($entry in $regEntries) {
        try {
            if ($entry.Name -eq "") {
                cmd /c "reg add `"$($entry.Path)`" /ve /d `"$($entry.Value)`" /f" | Out-Null
            } else {
                cmd /c "reg add `"$($entry.Path)`" /v `"$($entry.Name)`" /d `"$($entry.Value)`" /f" | Out-Null
            }
...
foreach ($ext in $extensions) {
    try {
        cmd /c "reg add `"HKCU\Software\Classes\$ext`" /ve /d `"$progId`" /f" | Out-Null
